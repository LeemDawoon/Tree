...
$( document ).ready(function () {
  ...
  for (var i=0, len=group_list.length; i<len; i++) {
    var addedHTML = '<li class="row">'+
                      ...
                      '<div class="col-xs-8 col-md-10 ">'+
                        '<h3>'+group_list[i].g_name;
    
    if (group_list[i].g_is_open==1) {
      addedHTML += "<a class='btn btn-default btn-xs pull-right' href='/member?g_id="+group_list[i].g_id+"'>Join</a>";
    }
    addedHTML +='<small><p class="text-right"><i class="fa fa-users"></i> '+group_list[i].g_count+'</p></small></h3>'+
  ...
  for (var i=0, len=member_list.length; i<len; i++) {
    var addedHTML = '<li class="row" >'+
                      "<a href='/profile/"+ member_list[i].u_id +"'>"+
                        '<div class="col-xs-4 col-md-2 ">'+
